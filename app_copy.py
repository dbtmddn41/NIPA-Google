...
def handle_user_msg(obj):
    if 'user_id' not in session or 'chat_id' not in session:
        # 로그인하지 않은 사용자의 경우 session 초기화 및 에러 처리
        session.clear()
        emit('alert', {'data': '로그인이 필요합니다.'})
        return
    else:
        print(obj)
        user_id = session['user_id']
...
def handle_disconnect():
    if 'user_id' in session and 'chat_id' in session:
        ...
        try:
            ...
            messages = cursor.execute('SELECT MESSAGE FROM MESSAGE_TABLE WHERE CHAT_ID = :CHAT_ID ORDER BY CREATED_AT', {'CHAT_ID': chat_id}).fetchall()
            print('chat_id_3:',chat_id)
            messages = map(lambda x: x[0], messages)
            messages = ' '.join(messages)

            chat_vector = text_embedding_bge(messages)
            chat_vector = json.dumps(chat_vector.tolist())

            cursor.execute('UPDATE CHAT_TABLE SET CHAT_VECTOR = :CHAT_VECTOR WHERE CHAT_ID = :CHAT_ID', {'CHAT_VECTOR': chat_vector, 'CHAT_ID': chat_id})
            connection.commit()
        except oracledb.DatabaseError as e:
            print(f"Database error occurred: {e}", file=sys.stderr)
            if connection:
